...
/**
 * AudioViewModelOld class to handle audio files , load them from running device
 * get there art
 */
class AudioViewModelOld: ViewModel() {
    ...
    fun loadAudioFiles(contentResolver: ContentResolver): List<AudioFile> {
        ...
        cursor?.use {
            ...
            while (it.moveToNext()) {
                ...
                audioList.add(AudioFile(id, title, artist, album, data, null))
            }
        }
        return audioList
    }
    ...
    fun createNotification(
        ...
        val playIntent = Intent(context, PlayBackServiceOld::class.java).apply {
            action = "ACTION_PLAY"
        }
        val pauseIntent = Intent(context, PlayBackServiceOld::class.java).apply {
            action = "ACTION_PAUSE"
        }
        val nextIntent = Intent(context, PlayBackServiceOld::class.java).apply {
            action = "ACTION_NEXT"
        }
        val previousIntent = Intent(context, PlayBackServiceOld::class.java).apply {
